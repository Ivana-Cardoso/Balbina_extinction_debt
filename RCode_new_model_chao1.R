...
# To fit the extinction debt model, I will use Chao1 estimator instead of 
# observed species richness
rownames(Balbina_islands) <- Balbina_islands$Ilha_Ano
data_chao <- as.data.frame(t(Balbina_islands[,c(9:151)]))
data_chao <- Filter(function(x) sum(x) != 0, data_chao)
chao1 <- ChaoRichness(data_chao, datatype = "abundance")
chao1$site <- rownames(chao1)

data_debt <- merge(data_debt, chao1, by = "site", all.x = TRUE)
data_debt <- data_debt[,-c(8,10:13)]
colnames(data_debt)[8] <- "chao1"
data_debt$chao1[is.na(data_debt$chao1)] <- 0

Balbina_m1 <- subset(data_debt, data_debt$chao1 > 0) #two islands has 0 species
m1 = lm(log10(chao1) ~ log10(a), data = Balbina_m1)
summary(m1)
...
Balbina_m1$ilhas <- sub("(_[A-Za-z])?_[0-9]{4}$", "", Balbina_m1$site)
...
data_debt[data_debt$chao1 <= 0, ]
Balbina_m1 <- data_debt[data_debt$chao1 > 0, ]

debt_ne <- nlme(
  chao1 ~ c * A^z + (s_prevista - c * A^z) * exp(-t / (k0 * A^alpha)),
  data = Balbina_m1,
...
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
png("debt_paid_area_chao1.png", width = 14, height = 10,units = "cm", res = 900)
...
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
ggsave("biogeograp_model_chao1.png",
...
biogeograp_model_plot_ref
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
ggsave("biogeograp_model_chao1_reference.png",
...
data_chao_11 <- as.data.frame(t(Balbina_11_islands[,c(9:151)]))
data_chao_11 <- Filter(function(x) sum(x) != 0, data_chao_11)
chao1 <- ChaoRichness(data_chao_11, datatype = "abundance")
chao1$site <- rownames(chao1)

data_debt_11 <- merge(data_debt_11, chao1, by = "site", all.x = TRUE)
data_debt_11 <- data_debt_11[,-c(8,10:13)]
colnames(data_debt_11)[8] <- "chao1"
data_debt_11$chao1[is.na(data_debt_11$chao1)] <- 0

Balbina_m1 <- subset(data_debt_11, data_debt_11$chao1 > 0) #two islands has 0 species
m1 = lm(log10(chao1) ~ log10(a), data = Balbina_m1)
summary(m1)
...
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
png("debt_paid_area_chao11.png", width = 14, height = 10,units = "cm", res = 900)
plot(debt_paid ~ log10(richness_2026$A),
     main = "Paid extinction debt over 39 years of island creation",
     xlab = "Island area (ha)",
     ylab = "Extinction debt paid (%)",
...
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
ggsave("11_biogeograp_model_chao1.png",
       plot = biogeograp_model_plot,
       width = 24,
       height = 12,
       units = "cm",
...
biogeograp_model_plot_ref
setwd("C:/Users/ivana/OneDrive/PhD_INPA/3.Extinction_debt/Balbina_cap.3/Figures")
ggsave("11_biogeograp_model_chao1_reference.png",
       plot = biogeograp_model_plot_ref,
       width = 24,
       height = 12,
       units = "cm",
